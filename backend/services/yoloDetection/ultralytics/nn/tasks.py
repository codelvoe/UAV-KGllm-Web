...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            {Detect, SiamDetect, WorldDetect, YOLOEDetect, Segment, YOLOESegment, Pose, OBB, ImagePoolingAttn, v10Detect}
        ):
        ...
        # ----------------------EdgeGradientAttention改造----------------------
        elif m is EdgeGradientAttention:
            # EdgeGradientAttention 需要两个输入: [img_rgb, feat]
            # 输出通道数 = feat 的通道数 (通常是 from 中最后一个索引的通道数)
            if isinstance(f, (list, tuple)):
                feat_idx = f[-1]
            else:
                feat_idx = f
            c2 = ch[feat_idx]  # 输出通道 = 特征输入通道
            # YAML 中只传 attn_ratio（或不传，默认 0.5），由这里统一构造 (c_feat, attn_ratio)
            if len(args) == 0:
                attn_ratio = 0.5
            else:
                attn_ratio = float(args[0])
            args = [c2, attn_ratio]
