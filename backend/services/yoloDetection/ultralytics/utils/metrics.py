...
def wasserstein_distance(mu1, mu2, sigma1, sigma2):
    """
    Calculate Wasserstein distance between two Gaussian distributions (NWD).

    Args:
        mu1 (torch.Tensor): Mean of the first distribution (center coordinates), shape (N, 2).
        mu2 (torch.Tensor): Mean of the second distribution (center coordinates), shape (N, 2).
        sigma1 (torch.Tensor): Standard deviation of the first distribution (width/2, height/2), shape (N, 2).
        sigma2 (torch.Tensor): Standard deviation of the second distribution (width/2, height/2), shape (N, 2).
    ...
    w_dist = center_distance + wh_distance
    
    # Normalize (NWD)
    # Using constant C=12.5 (typical for NWD) or dynamic based on object size
    # Reference implementation often uses: constant or size-dependent
    # Let's use a standard constant for now, or the one from the paper if available.
    # The user's reference code had: nwd = 1.0 - torch.exp(-w_dist.sqrt() / norm_factor)
    # where norm_factor = 5.0 * (target_sigma.mean(dim=-1) + eps)
    
    # We need target_sigma here. But the function signature only has sigma2.
    # Assuming sigma2 is target (based on usage in loss.py: wasserstein_distance(mu_p, mu_t, sigma_p, sigma_t))
    
    norm_factor = 5.0 * (sigma2.mean(dim=1) + 1e-7)
    nwd = 1.0 - torch.exp(-torch.sqrt(w_dist) / norm_factor)
    return nwd


def shape_aware_penalty(pred_wh, target_wh, ratio_power=1.2, gamma_scale=0.3, eps=1e-7):
    """
    Calculate Shape-Aware Penalty for SSQ Loss.

    Args:
        pred_wh (torch.Tensor): Predicted width and height, shape (N, 2).
        target_wh (torch.Tensor): Target width and height, shape (N, 2).
        ratio_power (float): Power for aspect ratio weight normalization.
        gamma_scale (float): Scaling factor for the penalty.
        eps (float): Small value to avoid division by zero.

    Returns:
        (torch.Tensor): Shape-aware penalty term.
    """
    # Aspect ratio weight normalization
    w_pow = torch.pow(target_wh[..., 0], ratio_power)
    h_pow = torch.pow(target_wh[..., 1], ratio_power)
    ...
    return (ww * omiga_w.pow(2) + hh * omiga_h.pow(2)) * gamma_scale


def dynamic_wiou(iou, alpha=2.5, beta=None):
    """
    Calculate Dynamic WIoU (Weighted IoU).

    Args:
        iou (torch.Tensor): IoU values.
        alpha (float): Alpha parameter for WIoU.
        beta (torch.Tensor, optional): Beta parameter. If None, it is calculated from IoU.

    Returns:
        (torch.Tensor): Dynamic WIoU loss term.
    """
    if beta is None:
        beta = (1 - iou).detach() / (1 - iou.mean().detach() + 1e-7)
    r = beta / (alpha ** (beta - 1 + 1e-3))  # Prevent alpha^0=1 case
    return r * (1 - iou) ** alpha
