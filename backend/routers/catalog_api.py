...
回答风格：
- 中文为主，表达自然，不要像固定模板；
- 用户问得简单时，直接回答；用户问得复杂时，分层说明；
- 可以主动澄清，也可以给出合理建议；
- 不要每次都机械复述系统背景；
- 不要过度限制为“只能谈项目”，但当问题涉及系统数据、算法、实验结论时，应优先结合下方项目上下文。

项目上下文：
- radar tracks: 294，主要为空间/运动候选，虚警较多；
- spectrum tracks: 2，提供 DJI Air3 / drone 等无线电语义证据；
- recognize tracks: 24，提供视觉辅助证据；
- decrypt label tracks: 1，仅用于评价，不进入 LLM 裁决；
...
3. 如果项目数据中没有依据，要明确说“不确定”或“当前数据未显示”；
4. 可以基于已有事实做分析和建议，但要说明这是推断或改进建议；
5. 普通交流、解释术语、页面优化、写作建议等问题可以正常回答，不必强行套回项目。
""".strip()
...
def build_prompt(message: str, context_type: str = "", context_id: str = "") -> str:
    ...
    return (
        ...
        f"用户问题：{message}\n\n"
        "请根据用户问题自然回答。若与项目相关，结合项目上下文；若是普通问题，也可以正常沟通："
    )


def ollama_generate(message: str, context_type: str = "", context_id: str = "", model: str = "") -> dict[str, Any]:
...
def send(body: ChatBody):
    result = ollama_generate(body.message, body.context_type, body.context_id, body.model)
    ...
    return {
        ...
        "evidence": [
            ...
            {"type": "experiment", "field": "methods", "value": "Radar-only, KG Fusion, KG-Heuristic, KG+LLM Agent"},
        ],
        "suggested_questions": [
...
def delete_session(session_id: str):
    sessions = [item for item in read_json(DATA_DIR / "chat_sessions.json", []) if item["session_id"] != session_id]
    write_json(DATA_DIR / "chat_sessions.json", sessions)
    return {"success": True}
...
def ollama_status():
    settings = load_settings()
    url = (settings.get("ollama_url") or "http://localhost:11434/api/generate").replace("/api/generate", "/api/tags")
